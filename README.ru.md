...
- **Навигация:** колесо мыши и `↑`/`↓`/`PgUp`/`PgDn`/`Пробел` прокручивают страницу и листают у верхнего/нижнего края; `←`/`→`/`Home`/`End` листают напрямую; **бегунок** перематывает страницы (тянете — видите номер, отпускаете — переход). Клик по счётчику страниц — ввод номера.
