...
const Tesseract = require("tesseract.js");   // esbuild инлайнит в ocr-worker.js

const log = (...a) => process.stderr.write("[ocr-worker] " + a.join(" ") + "\n");
const [, , imagePath, langs] = process.argv;
const dir = __dirname;   // = корень папки плагина (ocr-worker.js лежит там)

(async () => {
  log("start image=", imagePath, "langs=", langs, "dir=", dir);
  if (!imagePath || !fs.existsSync(imagePath)) {
    process.stdout.write(JSON.stringify({ ok: false, error: "no image: " + imagePath })); process.exit(1);
  }
  const tdir = path.join(dir, "tesseract");
  const workerPath = path.join(tdir, "worker.min.js");
  const coreFile = (fs.existsSync(tdir) ? fs.readdirSync(tdir) : [])
    .filter((f) => /\.wasm\.js$/.test(f)).find((f) => !/simd/.test(f)) || "";
